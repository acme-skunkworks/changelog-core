...
2. **Enrich / finalise (post-merge)** — the estate's shared `reusable-changelog-enrich.yml` runs in the consumer repo and drives the `changelog-core` CLI: `changelog-core enrich` (deploy targets) fills the post-merge fields; `changelog-core finalise` (npm targets) also stamps the just-published `version`. For every entry without a `version` it resolves the merged PR from the `branch` field via `gh` (filling `merged_at`/`commit`/`pr`/`stats`) and rewrites Linear IDs to links. Idempotent and re-run-safe. **This repo is the exception** (see "## Repo"): its own `pkg-release.yml` does not wire this workflow, so its entries after v1.1.0 stay versionless.
