...
`@acme-studio/changelog-core` — the estate's shared changelog engine. A single, zero-runtime-dependency npm package (Node 22+) that ships two things: the changelog **contract** (`changelog-contract.md`, also importable as `@acme-studio/changelog-core/contract`) and a dual-runtime **CLI** (`changelog-core`) that runs the same logic from CI **and** local dev. It replaced the triplicated skill `.mjs` + per-repo `infrastructure/scripts` changelog copies (A-803) — the drift-prone status quo where every repo carried its own fork of the enrich/finalise/validate logic.
...
| `pkg-release.yml` → `release`           | ⚠️ remote   | Thin caller of `acme-studio/shared-workflows`'s `reusable-pkg-release.yml` (v1.0.2). `act` must **fetch the remote reusable workflow** (needs network + a `GITHUB_TOKEN`); its build+pack completes but the OIDC-bound publish/provenance and attestation steps can't run locally. This repo publishes for real (Release is **active**), so the decisive check is the actual push run, not `act`. |
