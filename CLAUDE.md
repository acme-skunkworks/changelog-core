...
- **Generates the skill configs** by wrapping the `initialise-skills` skill (per-skill `config.json` is gitignored/materialised locally under the agent-skills v1.1.0 generated-config model; only the neutral `config.example.json` ships in the vendored bundles). Runs **after** the skills pull so configs match the pulled versions.
