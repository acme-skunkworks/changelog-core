...
`@rheged-studio/changelog-core` — the estate's shared changelog engine. A single, zero-runtime-dependency npm package (Node 22+) that ships two things: the changelog **contract** (`changelog-contract.md`, also importable as `@rheged-studio/changelog-core/contract`) and a dual-runtime **CLI** (`changelog-core`) that runs the same logic from CI **and** local dev. It replaced the triplicated skill `.mjs` + per-repo `infrastructure/scripts` changelog copies (A-803) — the drift-prone status quo where every repo carried its own fork of the enrich/finalise/validate logic.

The CLI has seven subcommands — `validate | enrich | finalise | set-affected-packages | add-links | backfill-commits | check-completeness` — dispatched from `src/cli.ts` (see the root `README.md` for the table). Command modules live in `src/commands/`, pure helpers in `src/lib/`, and the published library surface is re-exported from `src/index.ts`.

The estate's shared `reusable-changelog-enrich.yml` calls this CLI **in-repo after merge** — `mode: enrich` (deploy targets) / `mode: finalise` (npm targets) — the model that superseded the old central, orchestrator-run finalise (A-801). `merge_strategy` is no longer part of the contract (A-802); unknown keys on existing entries still validate.

The published artifact is `dist/`, compiled from `src/`. The workflow/release shell (`.github/`, `infrastructure/`, `changelog/`, `release-please-config.json`, `.release-please-manifest.json`) is **not** published. A thin `changelog` agent skill (installed under `.claude/skills/` / `.agents/skills/`) delegates to this package rather than carrying its own copy of the logic.

> **This repo is the changelog engine's own exception.** `changelog-core`'s `pkg-release.yml` is publish-only and does **not** wire `reusable-changelog-enrich.yml`, so it does not self-enrich/self-stamp its own `changelog/` entries (the v1.1.0 bootstrap entries carry a `version`; entries merged since stay versionless). The in-repo enrich model above is what consumer repos run; this repo defines and tests the contract they consume.

## Decisions live in Linear, not ADRs

Architectural and process decisions for this repo are recorded as **Linear issues**, not as
in-repo ADR files. The issue IDs threaded through this document and the code comments (e.g.
`A-326`, `A-328`, `A-447`, `A-639`, `A-801`, `A-803`) are the durable decision record: follow
the ID to Linear for the full rationale. Capture new decisions as Linear issues and reference
their IDs in commits, PR bodies, and comments rather than adding a `docs/adr/` tree. The package
is catalogued in the Open Source initiative in Linear (A-238).
...
- **ESLint** — `eslint.config.ts` consumes `@rheged-studio/eslint-config`, composing the `base` stack plus the `typescript` overrides, then adds local override blocks: a `complexity: off` + `import/no-extraneous-dependencies` `devDependencies: true` override for `tests/**` / `infrastructure/**` (which legitimately import devDeps like vitest and fixtures), a `complexity: off` override for the CLI command/lib modules (`src/commands`/`src/lib`/`src/cli.ts`, branchy flat lists of schema checks), and the `tsconfig.eslint.json` project pin (see above). The preset also re-exports opt-in presets (`testing`, `frameworkRouting`, `astro`, `sanity`, `storybook`, `tableComponents`) — pull them in as needed. The config is authored in `.ts` (loaded by `jiti`, a devDependency ESLint v9.18+ requires for TypeScript config) and wrapped in `defineConfig` from `eslint/config`, so the whole array — including the two local override blocks — is type-checked against the preset's shipped types (`./dist/index.d.ts`) instead of only failing at lint-run time. `pnpm tsc` type-checks it via `tsconfig.tools.json`.
...
Secrets (`GITHUB_TOKEN`), OIDC Trusted Publishing, and release-please behaviour are unchanged — not in this file. **No bot key ships in this repo.**
...
- **Gate name (A-419 / A-596 / A-437).** Clacks polls the `GO/NO GO` check-run only. A-419 opened a dual-accept window (`🔬 Build & Lint` **or** `GO/NO GO`); A-596 collapsed it to `GO/NO GO`-only once every served repo emitted it, and A-437 retired the old gate role. The caller swap (A-447) had already **removed** the `🔬 Build & Lint` context from this repo — replaced by `lint / Lint` + `build-test / Build & Test`. `pr-title`'s name is _also_ the estate-pinned required-check context (A-405); don't tidy it.
...
| `pkg-release.yml` → `release`           | ⚠️ remote   | Thin caller of `rheged-studio/shared-workflows`'s `reusable-pkg-release.yml` (v1.0.2). `act` must **fetch the remote reusable workflow** (needs network + a `GITHUB_TOKEN`); its build+pack completes but the OIDC-bound publish/provenance and attestation steps can't run locally. This repo publishes for real (Release is **active**), so the decisive check is the actual push run, not `act`. |
...
`act` validates workflow _wiring_ — that the YAML resolves, steps fire in order, env propagates. It says nothing about whether the logic _inside_ a `run:` block is correct. `infrastructure/` is the home for the **shell** that logic distils to: tool-install and publish scripts extracted from workflow `run:` blocks, runnable and unit-tested in isolation via bats. (The changelog _TypeScript_ logic that used to live here has moved into `src/` as the `changelog-core` CLI — commands under `src/commands/`, pure libs under `src/lib/`, vitest-tested in the repo-root `tests/`.) The full conventions document is `infrastructure/README.md`; the high-level rules:
...
CI (A-447): the `build-test` reusable caller runs ShellCheck (`infrastructure/scripts` + the husky hooks), Vitest (the repo-root `tests/`), and bats (`pnpm exec bats`, so `bats` is a devDependency); the `lint` caller runs `validate:changelog`; the `changelog-completeness` job runs the completeness gate. Locally, `pnpm lint:sh` / `pnpm test:sh` skip with install hints if `shellcheck` / `bats` aren't on PATH — `pnpm test` (vitest) always runs because vitest is a node devDep.

> The changelog CLI's command modules are branchy flat lists of schema checks, and the `tests/` + `infrastructure/` helpers legitimately import devDependencies (vitest, fixtures), so `eslint.config.ts` scopes a `complexity: off` + `import/no-extraneous-dependencies` `devDependencies: true` override to `src/commands`/`src/lib`/`src/cli.ts` and `tests/**` / `infrastructure/**`. (changelog-core has **zero runtime dependencies** and its own frontmatter parser — no `gray-matter`.)

When adding workflow-extracted shell tooling, write the bats test first, then wire from YAML as a one-liner: `run: bash infrastructure/scripts/<name>.sh`. (The bespoke `/send-it` slash command and its `infrastructure/send-it/` helpers were superseded by the shared `send-it` agent skill — see "## Agent skills".)

## Dated changelog (`changelog/`)

The `changelog/` directory is the **only** changelog in the repo — there is no root `CHANGELOG.md` (release-please runs with `skip-changelog`, A-371). It keeps **one dated Markdown file per PR** — a browsable, per-change, machine-readable record (the `version` field, stamped at release only on release-triggering entries, ties an entry back to the published release it shipped in). The reusable release workflow (called by `pkg-release.yml`) sources its GitHub-release notes from the version-stamped entries. Full schema and lifecycle in **`changelog/README.md`**; entries are written by `/send-it`.

Two-stage lifecycle — authored in the feature PR, enriched in-repo after merge (A-801):

1. **PR-time** — `/send-it` writes `changelog/<YYYYMMDD-HHMMSS>-<slug>.md` with the PR-time fields (and empty enrichment placeholders) for **every** PR under the v1.1.0 "record everything, filter later" model — a non-release entry simply stays version-less and is filtered out of release notes. The entry merges to `main` with its PR and sits with placeholders until enriched. CI's changelog-completeness gate enforces the one hard coupling: a release-triggering `feat`/`fix`/breaking PR title **must** carry an entry.
2. **Enrich / finalise (post-merge)** — the estate's shared `reusable-changelog-enrich.yml` runs in the consumer repo and drives the `changelog-core` CLI. `changelog-core enrich` (deploy targets) is **env-driven** — it fills `merged_at`/`commit`/`pr`/`stats` from PR data the workflow passes as env vars, leaving `stats.commits` and `version` for release time and rewriting no links. `changelog-core finalise` (npm targets) is the **release-time** path — it resolves the merged PR via `gh` (full `stats` including `commits`), stamps the just-published `version`, and rewrites bare Linear IDs to links. Both are fill-once and re-run-safe. **This repo is the exception** (see "## Repo"): its own `pkg-release.yml` does not wire this workflow, so its entries after v1.1.0 stay versionless.

`validate:changelog` (`changelog-core validate`) enforces the schema (CI: the `lint` reusable caller's changelog lane). Required frontmatter is relaxed to `title`/`created_at`/`category`/`breaking` so backfilled historical entries and in-flight entries both pass. The CLI is `changelog-core` (`src/cli.ts`) with seven subcommands; `finalise` composes the pure `enrich` / `add-links` / `stamp` library modules under `src/lib/`.

## Release workflow

> **Live on this repo.** `changelog-core` is a real, publishing npm package, so the Release workflow is **active** here (`gh workflow list` shows `Release` as `active`) — a push to `main` that merges a freshly bumped, untagged version publishes to npm. Everything below describes that workflow as it runs.
>
...
2. After merge, **Clacks** (road-runner-bot, runs a 15-min cron) mints a short-lived repo-scoped App token, runs `release-please release-pr` (which ranks Conventional Commits on `main` — A-824 — and writes `package.json` + `.release-please-manifest.json`), pushes the `release-please--branches--main` branch, and opens the "`chore(main): release <version>`" release PR. Changelog enrichment is **no longer** an orchestrator step — it runs in-repo via the shared `reusable-changelog-enrich.yml` (A-801). On a later tick the orchestrator **squash-merges** that release PR once the CI gate is green — the `GO/NO GO` check-run (see "CI gate (`GO/NO GO`)" above). Fan-out PRs also stay squash.
...
**The publish leg calls npm directly (no `changesets/action`).** The reusable workflow inlines the raw-npm publish — `npm publish "$TARBALL" --access public --provenance` on the prebuilt tarball (A-328) — rather than delegating to `pnpm`'s publish path. This repo's `infrastructure/scripts/publish-via-raw-npm.sh` is the unit-tested **reference** for that logic (no longer CI-invoked since the reusable-workflow migration, like the `ensure-*.sh` scripts). Two reasons shaped it (both diagnosed in A-174):
