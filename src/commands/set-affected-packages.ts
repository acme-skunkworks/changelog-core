...
//
// The post-merge-only fields (`merged_at`, `commit`, and authoritative `stats`)
// are deliberately NOT touched here — they're owned by the release-orchestrator
// and stay blank until it fills them.
//
...
import { derivePackagesFromPaths } from "../lib/derive-packages.js";
 ...
 */
export function buildAffectedPackagesFrontmatter(
...
Usage:
  changelog-core set-affected-packages            Derive + write affected_packages (writes; needs git)
  changelog-core set-affected-packages --check    Report what would change; write nothing (alias: --dry-run)
  changelog-core set-affected-packages --self-test  Run the built-in offline smoke test
  changelog-core set-affected-packages --help     Show this message (alias: -h)`;
