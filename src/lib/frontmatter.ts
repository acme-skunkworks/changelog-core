...
// package. It is NOT a general YAML implementation — it handles exactly the
...
function parseScalar(token: string): FrontmatterValue {
  ...
  if (text.startsWith('"') && text.endsWith('"') && text.length >= 2) {
    ...
    return text
      .slice(1, -1)
      .replaceAll(/\\(.)/g, (_, char: string) => (char === "n" ? "\n" : char));
  }
...
function parseInlineArray(body: string): FrontmatterValue[] {
...
function parseBlockScalar(indicator: string, block: string[]): string {
...
function parseMapping(lines: string[], startIndent: number): FrontmatterData {
...
function reparsesAsNonString(string_: string): boolean {
...
function needsQuoting(string_: string): boolean {
  if (string_.length === 0) {
    return true;
  }
...
function serialiseString(string_: string): string {
