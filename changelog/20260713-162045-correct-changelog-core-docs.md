...
- Rewrote `changelog/README.md`'s Lifecycle section from the retired
  ...
  `mode: finalise`) driving the `changelog-core` CLI after merge ([A-801](https://linear.app/rheged-studio/issue/A-801)). Fixed
...
- Replaced the copied `npm-package-template` boilerplate in `CLAUDE.md` that
  ...
  and remaining `npm-package-template` references ([A-803](https://linear.app/rheged-studio/issue/A-803), [A-802](https://linear.app/rheged-studio/issue/A-802)).
